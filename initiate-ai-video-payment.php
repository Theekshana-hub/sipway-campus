...
if (!isset($_SESSION['student_id'])) {
    header('Location: practice-ai-video.php?error=not_logged_in');
    exit;
}
...
// Check existing active package -- CORRECT TABLE: activated_ai_video_packages
$stmt = $conn->prepare("
    SELECT id FROM activated_ai_video_packages 
...
if ($stmt->get_result()->num_rows > 0) {
    $stmt->close();
    header('Location: practice-ai-video.php?error=already_active');
    exit;
}
$stmt->close();

// Unique order ID
$orderId = 'AIVIDEO-' . $studentId . '-' . $packageId . '-' . time();
$amount  = (float)$pkg['price'];
$cents   = dfcc_amount_to_cents($amount);

// Save initial transaction record (payment_transactions + payments tables)
$packageType = 'ai_video';

$stmt1 = $conn->prepare("
    INSERT INTO payment_transactions 
    (student_id, package_id, package_type, order_id, amount, currency, status, created_at)
    VALUES (?, ?, ?, ?, ?, 'LKR', 'initiated', NOW())
");
$stmt1->bind_param('iissd', $studentId, $packageId, $packageType, $orderId, $amount);
$stmt1->execute();
$stmt1->close();
...
// Insert into activated_ai_video_packages -- reference_no is NOT NULL, must be provided
$stmt3 = $conn->prepare("
    INSERT INTO activated_ai_video_packages
    (student_id, package_id, status, payment_method, reference_no, payment_status, amount, order_id)
    VALUES (?, ?, 'pending', 'online', ?, 'pending', ?, ?)
");
$stmt3->bind_param('iisds', $studentId, $packageId, $orderId, $amount, $orderId);
$stmt3->execute();
$stmt3->close();
...
$requestData = [
    ...
    'cssLocation2'   => '',
    'useReliability' => true,
    'extraData'      => ['package_type' => 'ai_video'],
];
...
if (!$result['ok'] || empty($result['body']['responseData']['paymentPageUrl']) || empty($result['body']['responseData']['reqid'])) {
    error_log('[DFCC] PAYMENT_INIT failed for ai_video order ' . $orderId . ': ' . ($result['error'] ?? $result['raw']));
    ...
    $fail3 = $conn->prepare("UPDATE activated_ai_video_packages SET status = 'rejected', payment_status = 'failed' WHERE order_id = ?");
    $fail3->bind_param('s', $orderId);
    $fail3->execute();
    $fail3->close();

    $conn->close();

    header('Location: practice-ai-video.php?error=payment_gateway_error');
    exit;
}
...
$upd3 = $conn->prepare("UPDATE activated_ai_video_packages SET gateway_reqid = ? WHERE order_id = ?");
$upd3->bind_param('ss', $reqId, $orderId);
$upd3->execute();
$upd3->close();
