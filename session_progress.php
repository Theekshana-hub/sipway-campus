...
// Packages progress (logged-in only)
if ($isLoggedIn && $studentId > 0) {
    $stmt2 = $conn->prepare("
        SELECT id, package_name, total_sessions, sessions_remaining, status, activated_at
    ...
    $stmt2->close();
    foreach (array_reverse($packages) as $p) {
        if ($p['status'] === 'active') {
            $activePackage = $p;
            break;
        }
    }
    if (!$activePackage && !empty($packages)) {
        $activePackage = $packages[0];
    }
    $totalSessions = $activePackage ? (int)$activePackage['total_sessions'] : 0;
    $sessionsLeft  = $activePackage ? (int)$activePackage['sessions_remaining'] : 0;
    $sessionsDone  = $totalSessions - $sessionsLeft;
    $percentDone   = $totalSessions > 0 ? round(($sessionsDone / $totalSessions) * 100) : 0;
    $ringOffset    = $ringCircumference * (1 - ($percentDone / 100));
}
...
<title>My Progress - Sipway Campus</title>
...
.page-title {
  ...
  letter-spacing: -0.5px; margin: 0 0 24px 0; position: relative; z-index: 1;
}
.guest-banner {
...
.guest-banner p {
  flex: 1; min-width: 200px; font-size: 13.5px; font-weight: 700; color: #6b21a8;
}
.guest-banner button {
...
.guest-banner button:hover { filter: brightness(1.06); }
.progress-grid {
...
.panel {
  ...
  border-radius: var(--radius-lg); box-shadow: var(--shadow-card); padding: 26px;
}
...
.ring-svg circle.ring-bg { fill: none; stroke: var(--line-soft); stroke-width: 16; }
.ring-svg circle.ring-fg {
  fill: none; stroke: url(#ringGradient); stroke-width: 16; stroke-linecap: round;
...
.ring-center-label { font-size: 12px; font-weight: 700; color: var(--muted); margin-top: 2px; }
.ring-svg-holder { position: relative; width: 220px; height: 220px; }
.ring-center-text {
  position: absolute; inset: 0; display: flex; flex-direction: column;
  align-items: center; justify-content: center;
}
.pkg-name-tag { margin-top: 18px; font-size: 13.5px; font-weight: 800; color: var(--text); }
.pkg-sub-tag { font-size: 12px; color: var(--muted); font-weight: 600; margin-top: 2px; }
.stat-row { display: flex; gap: 12px; margin-top: 22px; width: 100%; }
.stat-box {
  flex: 1; background: var(--purple-soft); border-radius: var(--radius-md);
  padding: 12px 10px; text-align: center;
}
.stat-box.done { background: var(--success-soft); }
.stat-box .num { font-size: 19px; font-weight: 800; color: var(--text); }
...
.pkg-list-item:last-child { margin-bottom: 0; }
.mini-ring-holder { position: relative; width: 56px; height: 56px; flex-shrink: 0; }
.mini-ring-holder svg circle.ring-bg { fill: none; stroke: var(--line-soft); stroke-width: 6; }
.mini-ring-holder svg circle.ring-fg {
  fill: none; stroke: url(#miniRingGradient); stroke-width: 6; stroke-linecap: round;
  transform: rotate(-90deg); transform-origin: 50% 50%;
}
.mini-ring-pct {
  position: absolute; inset: 0; display: flex; align-items: center;
  justify-content: center; font-size: 11px; font-weight: 800; color: var(--text);
}
.pkg-list-info { flex: 1; min-width: 0; }
.pkg-list-name { font-size: 13.5px; font-weight: 800; color: var(--text); }
.pkg-list-meta { font-size: 11.5px; color: var(--muted); font-weight: 600; margin-top: 3px; }
.status-badge {
...
.status-badge.completed { background: var(--purple-soft); color: #6b21a8; }
...
<div class="shell">
  <aside class="sidebar" id="sidebar">
    ...
    <a href="session_progress.php" class="nav-item active">
...
<a href="student_chat.php" class="nav-item">
  ...
  <main class="main">
    <h1 class="page-title animate-up">My Progress</h1>
    <?php if (!$isLoggedIn): ?>
      <div class="guest-banner animate-up delay-1">
        <p>🔐 Login / Register කරලා ඔබේ package progress එක බලන්න!</p>
        <button type="button" onclick="openAuthModal(false)">Login / Register</button>
      </div>
      <div class="panel animate-up delay-1">
        <div class="empty-progress">
          Progress බලන්න login වෙන්න ඕනේ.<br>
          Login කරලා package activate කරලා sessions book කරන්න!
        </div>
      </div>
    <?php elseif (empty($packages)): ?>
      <div class="panel animate-up delay-1">
        <div class="empty-progress">
          Package එකක්වත් activate කරලා නැති නිසා progress එකක් නැහැ.<br>
          Package එකක් activate කරලා session book කරන්න!
        </div>
      </div>
    <?php else: ?>
      <div class="progress-grid">
          ...
          <div class="ring-wrap">
            <div class="ring-svg-holder">
              <svg class="ring-svg" width="220" height="220" viewBox="0 0 220 220">
                ...
                <circle class="ring-bg" cx="110" cy="110" r="<?php echo $ringRadius; ?>"></circle>
                <circle class="ring-fg" cx="110" cy="110" r="<?php echo $ringRadius; ?>"
                        stroke-dasharray="<?php echo $ringCircumference; ?>"
                        stroke-dashoffset="<?php echo $ringOffset; ?>"></circle>
              </svg>
              <div class="ring-center-text">
                <div class="ring-center-pct"><?php echo $percentDone; ?>%</div>
                <div class="ring-center-label">Completed</div>
              </div>
            </div>
            <p class="pkg-name-tag"><?php echo htmlspecialchars($activePackage['package_name']); ?></p>
            <p class="pkg-sub-tag">Activated on <?php echo date('d M Y', strtotime($activePackage['activated_at'])); ?></p>
            <div class="stat-row">
              <div class="stat-box done">
                <div class="num"><?php echo $sessionsDone; ?></div>
                <div class="lbl">Completed</div>
              </div>
              <div class="stat-box">
                <div class="num"><?php echo $sessionsLeft; ?></div>
                <div class="lbl">Remaining</div>
              </div>
              <div class="stat-box">
                <div class="num"><?php echo $totalSessions; ?></div>
        ...
        <div class="panel animate-up delay-2">
          ...
          <?php foreach ($packages as $p):
              $pTotal = (int)$p['total_sessions'];
              $pLeft = (int)$p['sessions_remaining'];
              $pDone = $pTotal - $pLeft;
              $pPct = $pTotal > 0 ? round(($pDone / $pTotal) * 100) : 0;
              $pCirc = 2 * M_PI * 24;
              $pOffset = $pCirc * (1 - ($pPct / 100));
          ?>
          <div class="pkg-list-item">
            <div class="mini-ring-holder">
              <svg width="56" height="56" viewBox="0 0 56 56">
                <circle class="ring-bg" cx="28" cy="28" r="24"></circle>
                <circle class="ring-fg" cx="28" cy="28" r="24"
                        stroke-dasharray="<?php echo $pCirc; ?>"
                        stroke-dashoffset="<?php echo $pOffset; ?>"></circle>
              </svg>
              <div class="mini-ring-pct"><?php echo $pPct; ?>%</div>
            </div>
            <div class="pkg-list-info">
              <div class="pkg-list-name"><?php echo htmlspecialchars($p['package_name']); ?></div>
              <div class="pkg-list-meta">
                <?php echo $pDone; ?> of <?php echo $pTotal; ?> sessions done · Activated <?php echo date('d M Y', strtotime($p['activated_at'])); ?>
              </div>
            </div>
    ...
    loginForm.addEventListener('submit', async function(e) {
      ...
      try {
        ...
        if (result.success) {
          ...
          setTimeout(() => { window.location.href = 'session_progress.php'; }, 600);
        } else {
